...
int main(void){
     ...
     * receiver will use. */
    q.cmd=DJP2_SWO_CONFIG; q.len=8;
    dj_put_le32(q.payload + 0, 2000000u);
    dj_put_le32(q.payload + 4, 0u);
    assert(djp2_dispatch(&q,&r)==0);
    q.cmd=DJP2_SWO_START; q.len=0;
    assert(djp2_dispatch(&q,&r)==0);
    assert(dj_swo_mock_feed((const uint8_t *)"ITM", 3)==0);
    q.cmd=DJP2_SWO_STATUS; q.len=0;
    assert(djp2_dispatch(&q,&r)==0);
    assert(r.len==20 && dj_le32(r.payload+8)==3 && r.payload[16]==1);
    q.cmd=DJP2_SWO_READ; q.len=2;
    dj_put_le16(q.payload, 16);
    assert(djp2_dispatch(&q,&r)==0);
    assert(r.len==3 && memcmp(r.payload,"ITM",3)==0);
    q.cmd=DJP2_SWO_STOP; q.len=0;
    assert(djp2_dispatch(&q,&r)==0);



    /* Vendor feature implementation pack: generic bridge modes and power trace. */
    q.cmd=DJP2_BRIDGE_INFO; q.len=0;
    assert(djp2_dispatch(&q,&r)==0);
    assert(r.len>10);
    q.cmd=DJP2_BRIDGE_GPIO; q.len=2; q.payload[0]=0; q.payload[1]=DJ_PIN_DATA0;
    assert(djp2_dispatch(&q,&r)==-EPERM);
    arm_safety(DJ_SAFETY_BRIDGE, 2);
    assert(djp2_dispatch(&q,&r)==0);
    assert(r.len==1);
    q.cmd=DJP2_BRIDGE_SPI; q.len=12;
    q.payload[0]=0; q.payload[1]=DJ_PIN_RESET; dj_put_le16(q.payload+2,0); dj_put_le32(q.payload+4,100000); dj_put_le16(q.payload+8,2);
    q.payload[10]=0x9f; q.payload[11]=0x00;
    assert(djp2_dispatch(&q,&r)==0);
    assert(r.len==2);
    q.cmd=DJP2_POWER_TRACE; q.len=8;
    dj_put_le16(q.payload+0,3); dj_put_le16(q.payload+2,0); dj_put_le32(q.payload+4,0);
    assert(djp2_dispatch(&q,&r)==0);
    assert(r.len==48);
    ...
    struct dj_target_cfg sl={.proto=DJ_PROTO_TI_SIMPLELINK_SWD,.clock_hz=1000000,.power=DJ_PWR_EXTERNAL};
    ...
    q.cmd=DJP2_DEBUG_DETACH; q.len=0;
    assert(djp2_dispatch(&q,&r)==0);
    struct dj_target_cfg slc={.proto=DJ_PROTO_TI_SIMPLELINK_CJTAG,.clock_hz=500000,.power=DJ_PWR_EXTERNAL};
    strcpy(slc.device, "CC1352P7");
    assert(dj_select_backend(&slc)==0);
    q.cmd=DJP2_ENTER; q.len=0;
    assert(djp2_dispatch(&q,&r)==0);
    q.cmd=DJP2_RAW_XFER; q.len=3; q.payload[0]=0x01; q.payload[1]=8; q.payload[2]=0xff;
    assert(djp2_dispatch(&q,&r)==0);
    assert(r.len==1);

...
DirtyJTAG exposes generic RTT helpers for backends that can read and write target memory. ARM SWD/JTAG OpenOCD remote-bitbang users should normally use OpenOCD's own RTT commands, because OpenOCD owns target memory transactions in that mode.

| Command | ID | Request | Response |
|---|---:|---|---|
| `DJP2_RTT_SCAN` | `0x60` | `start:u32 end:u32` | `control_block:u32` |
| `DJP2_RTT_INFO` | `0x61` | `control_block:u32` | 15 `u32` RTT descriptor fields |
| `DJP2_RTT_READ` | `0x62` | `control_block:u32 channel:u8 max_len:u16` | up-buffer bytes |
| `DJP2_RTT_WRITE` | `0x63` | `control_block:u32 channel:u8 bytes...` | `written:u32` |

See `docs/SEGGER_RTT.md` for the control-block layout, host examples, and backend limitations.

## Rev O SEGGER-inspired trace commands

### RTT channel descriptor command

| Command | ID | Request | Response |
|---|---:|---|---|
| `DJP2_RTT_CHANNEL_INFO` | `0x64` | `control_block:u32 direction:u8 channel:u8` where direction `0=up`, `1=down` | `direction:u8 channel:u8 name_len:u8 rsv:u8 name_addr:u32 buffer:u32 size:u32 wr:u32 rd:u32 flags:u32 used:u32 free:u32 name_bytes...` |

This extends the earlier RTT API from channel-0-only summary to arbitrary up/down channel inspection. It is intended for RTT Viewer-like channel discovery and for selecting binary/profiling channels.

### SWO/ITM capture control-plane commands

| Command | ID | Request | Response |
|---|---:|---|---|
| `DJP2_SWO_CONFIG` | `0x68` | `baud:u32 flags:u32` | empty |
| `DJP2_SWO_START` | `0x69` | empty | empty |
| `DJP2_SWO_STOP` | `0x6a` | empty | empty |
| `DJP2_SWO_READ` | `0x6b` | `max_len:u16` | captured bytes |
| `DJP2_SWO_STATUS` | `0x6c` | empty | `baud:u32 flags:u32 available:u32 dropped:u32 active:u8 rsv[3]` |

Rev O provides the stable USB/host interface and a tested firmware ring-buffer model. Real SWO signal capture must be backed by an RP2040 PIO/UART receiver before being treated as production hardware capture.


## Rev Q bridge and power trace commands

| Command | ID | Payload | Response |
|---|---:|---|---|
| `DJP2_BRIDGE_INFO` | `0x46` | empty | ASCII capability list |
| `DJP2_BRIDGE_GPIO` | `0x47` | `op:u8 role:u8 [value:u8]` | optional value byte |
| `DJP2_BRIDGE_SPI` | `0x48` | `mode:u8 cs_role:u8 flags:u16 hz:u32 len:u16 tx[]` | rx bytes |
| `DJP2_BRIDGE_I2C` | `0x49` | `addr:u8 txlen:u16 rxlen:u16 hz:u32 tx[]` | rx bytes |
| `DJP2_BRIDGE_UART` | `0x4a` | `baud:u32 flags:u16 txlen:u16 rxlen:u16 tx[]` | rx bytes |
| `DJP2_POWER_TRACE` | `0x4b` | `samples:u16 interval_ms:u16 flags:u32` | 16-byte sample records |

Power trace sample record: `t_ms:u32 vtarget_mv:u32 vpp_mv:u32 itarget_ma_or_fault:u32`, where bit31 of the current field is the power-fault flag.

Protocol IDs `80` and `81` are reserved for TI SimpleLink CC13xx/CC26xx SWD and cJTAG backends.

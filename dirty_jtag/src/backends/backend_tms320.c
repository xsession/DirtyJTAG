...
static void dly(void)
{
    ...
    if (h && h->delay_us) h->delay_us(hp());
}

static int sel(const struct dj_target_cfg *c)
{
    if (!c) return -EINVAL;
    if (c->power == DJ_PWR_5V) return -ERANGE; /* XDS110-class target I/O is 1.8..3.6 V. */
    cfg = *c;
    return 0;
}
...
static int jtag_clk(bool tms, bool tdi, bool *tdo)
{
    ...
    r = h->write(DJ_PIN_DATA0, tms); if (r) return r;   /* TMS */
    r = h->write(DJ_PIN_DATA1, tdi); if (r) return r;   /* TDI */
    dly();
    bool v = false;
    if (tdo) { r = h->read(DJ_PIN_DATA2, &v); if (r) return r; }
...
{
    int r;
    r = jtag_clk(true, true, NULL); if (r) return r;  /* Select-DR */
    r = jtag_clk(true, true, NULL); if (r) return r;  /* Select-IR */
    r = jtag_clk(false, true, NULL); if (r) return r; /* Capture-IR */
...
static int goto_shift_dr(void)
{
    int r;
    r = jtag_clk(true, true, NULL); if (r) return r;  /* Select-DR */
    r = jtag_clk(false, true, NULL); if (r) return r; /* Capture-DR */
...
{
    int r;
    r = jtag_clk(true, true, NULL); if (r) return r;  /* Exit1 then Update */
    return jtag_clk(false, true, NULL);               /* Run-Test/Idle */
}

static int shift_bits(bool ir, uint16_t bits, const uint8_t *tx, uint8_t *rx, size_t *rxlen)
{
    if (!bits || !tx || !rx || !rxlen) return -EINVAL;
    size_t bytes = (bits + 7u) / 8u;
    if (*rxlen < bytes) return -ENOSPC;
    memset(rx, 0, bytes);
    ...
    for (uint16_t i = 0; i < bits; ++i) {
        ...
        if (r) return r;
        if (tdo) rx[i / 8u] |= (uint8_t)(1u << (i & 7u));
    }
    r = finish_shift();
    if (!r) *rxlen = bytes;
    return r;
...
static int set_lines(bool nreset_released, bool ntrst_or_aux_released)
{
    const struct dj_hw_ops *h = dj_hw();
    if (!h) return -ENODEV;
    int r = h->write(DJ_PIN_RESET, nreset_released); if (r) return r;
...
static int enter(void)
{
    ...
    int r;
    r = h->dir(DJ_PIN_CLK, DJ_DIR_OUTPUT); if (r) return r;   /* TCK */
    r = h->dir(DJ_PIN_DATA0, DJ_DIR_OUTPUT); if (r) return r; /* TMS */
    r = h->dir(DJ_PIN_DATA1, DJ_DIR_OUTPUT); if (r) return r; /* TDI */
    r = h->dir(DJ_PIN_DATA2, DJ_DIR_INPUT); if (r) return r;  /* TDO */
    r = h->dir(DJ_PIN_RESET, DJ_DIR_OUTPUT); if (r) return r; /* nRESET */
    r = h->dir(DJ_PIN_AUX, DJ_DIR_OUTPUT); if (r) return r;   /* nTRST or EMU0 hint */
...
const struct dj_backend dj_backend_tms320_jtag = {
    DJ_PROTO_TMS320_C2000_JTAG,

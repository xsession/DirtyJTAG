...
static uint32_t baud(void) {
	uint32_t b = cfg.clock_hz ? cfg.clock_hz : 115200u;
	return b > 225000u ? 225000u : b;
}
static int enter(void) {
	...
	if ((cfg.flags & 1u) != 0) {
		if (!h->vpp_boost || !h->hv_data0_apply)
			return -ENOTSUP;
		int r = h->vpp_boost(true);
		if (r)
			return r;
		if ((r = h->hv_data0_apply(true)))
			return r;
		h->delay_us(1000);
		h->hv_data0_apply(false);
		h->delay_us(
		    50); /* begin UPDI synchronization/key traffic well inside vendor activation window */
	}
...
static int raw(const uint8_t *tx, size_t tn, uint8_t *rx, size_t *rn) {
	if (!tx || tn < 4 || !rn)
		return -EINVAL;
	uint16_t txc = (uint16_t)tx[0] | ((uint16_t)tx[1] << 8),
	         rxc = (uint16_t)tx[2] | ((uint16_t)tx[3] << 8);
	if (4u + (size_t)txc > tn || (size_t)rxc > *rn)
		return -EINVAL;
...
const struct dj_backend dj_backend_updi = {DJ_PROTO_AVR_UPDI,
                                           "avr-updi-8e2-raw",

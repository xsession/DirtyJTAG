...
static int enter(void) {
	...
	uint8_t mode = 0x3A;
	return dj_uart_1wire_txrx(&mode, 1, 0, 0, 9600, false);
}
...
static int raw(const uint8_t *t, size_t n, uint8_t *r, size_t *rn) {
	...
	if (4u + txc > n || rxc > *rn)
		return -EINVAL;
	int e = dj_uart_1wire_txrx(t + 4, txc, r, rxc, cfg.clock_hz ? cfg.clock_hz : 115200, false);
	if (!e)
		*rn = rxc;
	return e;
}
const struct dj_backend dj_backend_rl78 = {DJ_PROTO_RENESAS_RL78,
                                           "renesas-rl78-protocol-a",
                                           DJ_CAP_RAW_XFER | DJ_CAP_EXPERIMENTAL,
                                           115200,
                                           1000000,
                                           sel,
                                           enter,

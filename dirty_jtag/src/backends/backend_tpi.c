...
static uint32_t hz(void) {
	uint32_t x = cfg.clock_hz ? cfg.clock_hz : 100000u;
	return x > 250000u ? 250000u : x;
}
...
/* envelope: u16 tx_count,u16 rx_count, then bytes; each byte is framed start+8 data+even parity+2
 * stop */
...
const struct dj_backend dj_backend_tpi = {DJ_PROTO_AVR_TPI,
                                          "avr-tpi-8e2-raw",
                                          DJ_CAP_RAW_XFER | DJ_CAP_TARGET_POWER |
                                              DJ_CAP_EXPERIMENTAL,
                                          100000,
                                          250000,
                                          sel,

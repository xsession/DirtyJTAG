...
{
    if (mock) return 0;
    if (!cap.loaded) return -ENOTSUP;
    if ((cap.flags & need) != need) return -ENOTSUP;
    return 0;
}
...
{
    if (!data || len < 16u) return -EINVAL;
    if (le32(data + 0) != CAPS_MAGIC) return -EBADMSG;
    if (data[4] != CAPS_VERSION) return -ENOTSUP;
    if (data[7] > 8u || data[6] > DJ_DSPIC_DEBUG_REG_BYTES) return -ERANGE;
    cap.loaded = true;
    cap.family = data[5];
...
int dj_dspic_debug_attach(void)
{
    ...
    if (!mock && !(cap.flags & CAPS_FLAG_HAS_ATTACH_SCRIPT)) return -ENOTSUP;
    attached = true;
    halted = mock;
    return 0;
}
...
int dj_dspic_debug_reset(bool halt_after_reset)
{
    ...
    if (dj_icsp_hal() && dj_icsp_hal()->set_mclr_low) {
        r = dj_icsp_hal()->set_mclr_low(true); if (r) return r;
        dj_icsp_hal()->delay_us(1000);
        r = dj_icsp_hal()->set_mclr_low(false); if (r) return r;
    }
    halted = halt_after_reset;
    attached = true;
    return 0;
...
int dj_dspic_debug_read_regs(uint8_t *out, size_t *len)
{
    ...
    if (r) return r;
    size_t n = mock ? sizeof(mock_regs) : (cap.register_bytes ? cap.register_bytes : DJ_DSPIC_DEBUG_REG_BYTES);
    if (*len < n) return -ENOSPC;
    if (mock) memcpy(out, mock_regs, n);
    else memset(out, 0, n); /* reserved until DE register script is supplied */
    *len = n;
    return 0;
}
...
int dj_dspic_debug_write_regs(const uint8_t *in, size_t len)
{
    ...
    if (r) return r;
    if (mock) {
        if (len != sizeof(mock_regs)) return -EINVAL;
...
int dj_dspic_debug_set_breakpoint(uint32_t address, uint8_t type, uint8_t slot)
{
    ...
    if (slot >= maxbp) return -ERANGE;
    if (mock) { mock_bp[slot] = address; return 0; }
    return -ENOTSUP;
}
...
int dj_dspic_debug_clear_breakpoint(uint8_t slot)
{
    ...
    uint8_t maxbp = mock ? 4u : cap.hw_breakpoints;
    if (slot >= maxbp) return -ERANGE;
    if (mock) { mock_bp[slot] = 0; return 0; }
    return -ENOTSUP;
}

...
static int dspic_attach(void)
{
    ...
    if (!r) dspic_state = DJ_DEBUG_ATTACHED;
    return r;
}
...
static int dspic_detach(void)
{
    ...
    if (!r) dspic_state = DJ_DEBUG_DETACHED;
    return r;
}
...
static int dspic_halt(void)
{
    ...
    if (!r) dspic_state = DJ_DEBUG_HALTED;
    return r;
}
...
static int dspic_run(void)
{
    ...
    if (!r) dspic_state = DJ_DEBUG_RUNNING;
    return r;
}
...
static int dspic_reset(bool halt_after_reset)
{
    ...
    if (!r) dspic_state = halt_after_reset ? DJ_DEBUG_HALTED : DJ_DEBUG_RESET;
    return r;
}
...
static int stm8_attach(void)
{
    ...
    if (!r) stm8_state = DJ_DEBUG_ATTACHED;
    return r;
}
...
static int stm8_detach(void)
{
    ...
    if (!r) stm8_state = DJ_DEBUG_DETACHED;
    return r;
}
...
static int stm8_halt(void)
{
    ...
    if (!r) stm8_state = DJ_DEBUG_HALTED;
    return r;
}
...
static int stm8_run(void)
{
    ...
    if (!r) stm8_state = DJ_DEBUG_RUNNING;
    return r;
}
...
static int stm8_reset(bool halt_after_reset)
{
    ...
    if (!r) stm8_state = halt_after_reset ? DJ_DEBUG_HALTED : DJ_DEBUG_RESET;
    return r;
}
...
static int openocd_attach(void)
{
    ...
    if (!b || !(b->caps & DJ_CAP_DEBUG_OPENOCD)) return -ENOTSUP;
...
static const struct dj_debug_ops debug_ops[] = {
    {
        ...
        .caps = DJ_CAP_DEBUG_PHY | DJ_CAP_DEBUG_RUNCTRL | DJ_CAP_DEBUG_REGS |
            DJ_CAP_DEBUG_BREAK | DJ_CAP_EXPERIMENTAL,
...
const struct dj_debug_ops *dj_debug_ops_for(enum dj_proto_id proto)
{
    for (size_t i = 0; i < sizeof(debug_ops) / sizeof(debug_ops[0]); ++i) {
        if (debug_ops[i].proto == proto) return &debug_ops[i];
    }
    return NULL;
}
...
int dj_debug_get_info(struct dj_debug_info *info)
{
    ...
    if (ops->status) return ops->status(info);
    return 0;
}

...
static int sbw_slot(bool tms, bool tdi, bool *tdo)
{
    ...
    r = h->dir(DJ_PIN_DATA0, DJ_DIR_OUTPUT); if (r) return r;
    r = h->write(DJ_PIN_DATA0, tms); if (r) return r;
    r = h->write(DJ_PIN_CLK, false); if (r) return r; dly();
    r = h->write(DJ_PIN_CLK, true); if (r) return r; dly();

    r = h->write(DJ_PIN_DATA0, tdi); if (r) return r;
    r = h->write(DJ_PIN_CLK, false); if (r) return r; dly();
    r = h->write(DJ_PIN_CLK, true); if (r) return r; dly();
    ...
    if (tdo) { r = h->read(DJ_PIN_DATA0, &v); if (r) return r; }
    dly();
    if (tdo) *tdo = v;
    return 0;
}
...
{
    int r;
    r = sbw_slot(true, true, NULL); if (r) return r;
    r = sbw_slot(true, true, NULL); if (r) return r;
    r = sbw_slot(false, true, NULL); if (r) return r;
    return sbw_slot(false, true, NULL);
}
...
static int enter(void)
{
    ...
    int r;
    r = h->dir(DJ_PIN_CLK, DJ_DIR_OUTPUT); if (r) return r;   /* SBWTCK / TEST */
    r = h->dir(DJ_PIN_DATA0, DJ_DIR_OUTPUT); if (r) return r; /* SBWTDIO / RST */
...
const struct dj_backend dj_backend_sbw = {
    DJ_PROTO_MSP430_SBW,
    "msp430-sbw",

...
{
    uint32_t hz = cfg.clock_hz ? cfg.clock_hz : 200000u;
...
{
    int r;
    r = jtag_clk(true, true, NULL); if (r) return r;  /* Update-IR/DR */
    return jtag_clk(false, true, NULL);               /* Run-Test/Idle */
}
...
static int enter(void)
{
    ...
    r = h->dir(DJ_PIN_RESET, DJ_DIR_OUTPUT); if (r) return r; /* nRST */
    r = h->dir(DJ_PIN_AUX, DJ_DIR_OUTPUT); if (r) return r;   /* TEST / JTAG enable */
    h->write(DJ_PIN_CLK, false);
    h->write(DJ_PIN_DATA0, true);
    h->write(DJ_PIN_DATA1, true);
    /* Conservative MSP430 TEST/RST gate for devices with shared JTAG pins.
     * Board validation can tighten this into family-specific sequences. */
    h->write(DJ_PIN_RESET, false);
    h->write(DJ_PIN_AUX, false);
    h->delay_us(200);
    h->write(DJ_PIN_AUX, true);
    h->delay_us(200);
...
const struct dj_backend dj_backend_msp430_jtag = {
    DJ_PROTO_MSP430_JTAG,
    "msp430-jtag",

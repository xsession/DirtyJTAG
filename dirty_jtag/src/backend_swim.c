...
{
    if (!out || !len || *len < 2u) return -EINVAL;
...
{
    if (!tx || !rn || n < 1u) return -EINVAL;
    if (tx[0] == 0x03u) { *rn = 0; return dj_swim_system_reset(); }
    if (n < 6u) return -EINVAL;
    uint32_t addr = ((uint32_t)tx[1] << 16) | ((uint32_t)tx[2] << 8) | tx[3];
    uint16_t len = (uint16_t)tx[4] | ((uint16_t)tx[5] << 8);
    if (tx[0] == 0x01u) {
        if (!rx || *rn < len) return -ENOSPC;
        int r = dj_swim_read_mem(addr, rx, len);
        if (!r) *rn = len;
        return r;
    }
    if (tx[0] == 0x02u) {
        if (n != 6u + len) return -EINVAL;
        *rn = 0;
...
const struct dj_backend dj_backend_swim = {
    DJ_PROTO_STM8_SWIM,
    "stm8-swim",
    DJ_CAP_IDENTIFY | DJ_CAP_READ | DJ_CAP_WRITE | DJ_CAP_RAW_XFER |
        DJ_CAP_DEBUG_PHY | DJ_CAP_DEBUG_RUNCTRL | DJ_CAP_DEBUG_REGS |

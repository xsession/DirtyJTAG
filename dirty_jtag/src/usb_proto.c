...
{
    if (!f || !out || f->len > DJP2_MAX_PAYLOAD || cap < DJP2_HDR_SIZE + f->len) return 0;
    ...
    if (f->len) memcpy(out + 20, f->payload, f->len);
...
{
    if (!b || !f || n < 20 || dj_le32(b) != DJP2_MAGIC || dj_le16(b + 4) != DJP2_VERSION) return -EINVAL;
    uint32_t l = dj_le32(b + 14);
    if (l > DJP2_MAX_PAYLOAD || n != 20 + l) return -EMSGSIZE;
    uint16_t got = dj_le16(b + 18);
    uint8_t h[18];
    memcpy(h, b, 18);
    uint32_t crc = dj_crc32(h, 18, 0);
    crc = dj_crc32(b + 20, l, crc);
    if (got != (uint16_t)(crc ^ (crc >> 16))) return -EBADMSG;
    memset(f, 0, sizeof(*f));
    f->cmd = dj_le16(b + 6);
    f->seq = dj_le16(b + 8);
    ...
    f->len = l;
    if (l) memcpy(f->payload, b + 20, l);
    return 0;
}

static uint16_t maperr(int r)
{
    if (r == 0) return DJP2_OK;
    if (r == -EINVAL) return DJP2_E_ARG;
    if (r == -ENOTSUP || r == -ENOSYS || r == -ENODEV) return DJP2_E_UNSUPPORTED;
    if (r == -ERANGE || r == -EMSGSIZE) return DJP2_E_RANGE;
    if (r == -EBUSY) return DJP2_E_BUSY;
    if (r == -EPERM || r == -ETIMEDOUT) return DJP2_E_POWER;
    return DJP2_E_IO;
}

static int emit_measurement(uint8_t *p, size_t cap, size_t *len)
{
    if (!p || !len || cap < 16) return -ENOSPC;
    if (!dj_hw() || !dj_hw()->measure) return -ENOTSUP;
...
int djp2_dispatch(const struct djp2_frame *q, struct djp2_frame *r)
{
    ...
    switch (q->cmd) {
    case DJP2_HELLO: {
        const char *s = "DirtyJTAG Universal Programmer+Debugger;proto=2;board=rpi_pico;power=tps630702+tps2553";
    ...
    case DJP2_LIST_PROTOCOLS: {
        ...
        for (size_t i = 0; i < dj_backend_count(); ++i) {
            ...
            if (p + 12 + nl > DJP2_MAX_PAYLOAD) break;
    ...
    case DJP2_LIST_DEVICES: {
        ...
        for (size_t i = 0; i < dj_device_count(); ++i) {
            ...
            if (nl > 255 || p + 14 + nl > DJP2_MAX_PAYLOAD) break;
    ...
    case DJP2_CONFIG: {
        ...
        if (nl > sizeof(c.device) - 1 || 19 + nl > q->len) { rc = -EINVAL; break; }
    ...
    case DJP2_STATUS: {
        ...
        struct dj_measurement m = {0};
        if (dj_hw() && dj_hw()->measure) (void)dj_hw()->measure(&m);
        ...
        if (nl) memcpy(r->payload + 36, c->device, nl);
    ...
    case DJP2_LEAVE:
        ...
        break;
    case DJP2_IDENTIFY: {
        if (!b || !b->identify) { rc = -ENOTSUP; break; }
        size_t outn = DJP2_MAX_PAYLOAD;
        rc = b->identify(r->payload, &outn);
        if (!rc) r->len = (uint32_t)outn;
        break;
    }
    case DJP2_ERASE:
        rc = dj_safety_require(DJ_SAFETY_ERASE);
        if (!rc) rc = b && b->erase ? b->erase() : -ENOTSUP;
        break;
    case DJP2_READ: {
        if (!b || !b->read_mem || q->len < 8) { rc = -ENOTSUP; break; }
        uint32_t a = dj_le32(q->payload), n = dj_le32(q->payload + 4);
        if (n > DJP2_MAX_PAYLOAD) { rc = -ERANGE; break; }
    ...
    case DJP2_WRITE: {
        ...
        if (rc) break;
        if (!b || !b->write_mem || q->len < 8) { rc = -ENOTSUP; break; }
        uint32_t a = dj_le32(q->payload), n = dj_le32(q->payload + 4);
        if (n + 8u != q->len) { rc = -EINVAL; break; }
        rc = b->write_mem(a, q->payload + 8, n);
        break;
    }
    case DJP2_RAW_XFER: {
        if (!b || !b->raw_xfer) { rc = -ENOTSUP; break; }
        size_t outn = DJP2_MAX_PAYLOAD;
    ...
    case DJP2_POWER:
        ...
        if (rc) break;
        if (!dj_hw() || !dj_hw()->power || q->len < 1) { rc = -EINVAL; break; }
        rc = dj_hw()->power((enum dj_power_mode)q->payload[0]);
        break;
    case DJP2_MEASURE: {
        size_t n = 0;
        rc = emit_measurement(r->payload, DJP2_MAX_PAYLOAD, &n);
        if (!rc) r->len = (uint32_t)n;
        break;
    }
    case DJP2_SET_PINMAP: {
        if (q->len != DJ_PIN_COUNT) { rc = -EINVAL; break; }
        struct dj_pinmap pm = {0};
        for (unsigned i = 0; i < DJ_PIN_COUNT; ++i) pm.gpio[i] = q->payload[i];
    ...
    case DJP2_VPP: {
        ...
        if (rc) break;
        if (q->len < 2 || !dj_hw()) { rc = -EINVAL; break; }
        bool on = q->payload[1] != 0;
        if (q->payload[0] == 0) rc = dj_hw()->vpp_boost ? dj_hw()->vpp_boost(on) : -ENOTSUP;
        else if (q->payload[0] == 1) rc = dj_hw()->vpp_apply ? dj_hw()->vpp_apply(on) : -ENOTSUP;
        else if (q->payload[0] == 2) rc = dj_hw()->hv_data0_apply ? dj_hw()->hv_data0_apply(on) : -ENOTSUP;
        else rc = -EINVAL;
        break;
    }
    ...
    case DJP2_PHY_INFO: {
        ...
        r->payload[2] = 0u;
        r->payload[3] = 0u;
        if (nl) memcpy(r->payload + 4, name, nl);
    ...
    case DJP2_DEBUG_BP_SET:
        if (q->len != 6) rc = -EINVAL;
        else rc = dj_debug_set_breakpoint(dj_le32(q->payload), q->payload[4], q->payload[5]);
        break;
    case DJP2_DEBUG_BP_CLEAR:
        if (q->len != 1) rc = -EINVAL;
        else rc = dj_debug_clear_breakpoint(q->payload[0]);
        break;

    case DJP2_RTT_SCAN: {
        if (q->len < 8) { rc = -EINVAL; break; }
    ...
    case DJP2_RTT_CHANNEL_INFO: {
        ...
        if (!rc) {
            ...
            if (ci.name_len) memcpy(r->payload + 36, ci.name, ci.name_len);
    ...
    case DJP2_SWO_STATUS: {
        ...
        if (!rc) {
            ...
            r->payload[17] = 0u; r->payload[18] = 0u; r->payload[19] = 0u;
            r->len = 20;
        }
        break;
    }
    case DJP2_SWO_READ: {
        if (q->len < 2) { rc = -EINVAL; break; }

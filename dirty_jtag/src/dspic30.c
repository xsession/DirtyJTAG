...
static int op(uint32_t x) {
	return dspic_exec(x);
}
static int nop2(void) {
	int r = op(0);
	return r ? r : op(0);
}
...
static int enter30(const struct dj_device *dev) {
	...
	if ((r = dj_mclr_low(true)))
		return r;
	h->delay_us(1000);
	if ((r = dj_mclr_low(false)))
		return r;
	if ((r = dj_vpp_prepare(dev)))
		return r;
	if ((r = dj_vpp_apply(true)))
		return r;
	h->delay_us(4000);
	/* Figure 11-4: 10 us low pulse followed by 4 ms at VIHH. */
	if ((r = dj_vpp_apply(false)))
		return r;
	if ((r = dj_mclr_low(true)))
		return r;
	h->delay_us(10);
...
static int special_5011_preerase(void) {
	...
	if ((r = op(0x24008Au)))
		return r; /* MOV #0x4008,W10 */
	if ((r = op(0x883B0Au)))
		return r; /* MOV W10,NVMCON */
	if ((r = op(0x200F80u)))
		return r; /* MOV #0xF8,W0 */
	if ((r = op(0x880190u)))
		return r; /* MOV W0,TBLPAG */
	if ((r = op(0x200067u)))
		return r; /* MOV #6,W7 */
	if ((r = op(0xEB0300u)))
		return r; /* CLR W6 */
	...
	for (unsigned i = 0; i < 2; i++) {
		...
		if ((r = op(0x883B39u)))
			return r;
		if ((r = start_external_cycle(2000)))
			return r;
	}
	return 0;
}
...
static int erase30(const struct dj_device *dev) {
	int r = dspic_exit_reset(DSPIC_GOTO_100, 0, 0);
	if (r)
		return r;
	if (dev->flags & DJ_DEVF_SPECIAL_ERASE) {
		if ((r = special_5011_preerase()))
			return r;
	}
	if ((r = op(0x2407FAu)))
		return r; /* MOV #0x407F,W10 */
	if ((r = op(0x883B0Au)))
		return r;
	if ((r = unlock30()))
		return r;
...
static int write30_words(const struct dj_device *dev, uint32_t addr, const uint32_t *words,
                         size_t count) {
	if (!words || !count || (addr & 1u) || count != dev->row_words ||
	    (addr % (2u * dev->row_words)))
		return -EINVAL;
	if (addr + 2u * (uint32_t)count - 2u > dev->user_end_pc)
		return -ERANGE;
	int r = dspic_exit_reset(DSPIC_GOTO_100, 0, 0);
	if (r)
		return r;
	...
	if ((r = op(0x883B0Au)))
		return r;
	if ((r = dspic_set_tbl_address(addr, 7, 0x880190u)))
		return r;
	for (size_t i = 0; i < count; i += 4) {
		uint16_t w[6];
		dspic_pack4(&words[i], w);
		if ((r = dspic_load_wregs_0_5(w)))
			return r;
		if ((r = dspic_table_load4(2)))
			return r;
	}
	if ((r = unlock30()))
		return r;
	if ((r = start_external_cycle(4000)))
		return r; /* P12a max = 4 ms */
...
const struct dj_pic_backend dj_backend_dspic30 = {
    ...
    .read_id = read30_id,
    .erase = erase30,
    .read_words = read30_words,
    .write_words = write30_words,
};

...
{
    if (!in || !out || !outlen || inlen < 8) return -EINVAL;
    const struct dj_hw_ops *h = dj_hw();
    if (!h || !h->measure) return -ENOTSUP;
    ...
    for (uint16_t i = 0; i < samples; ++i) {
        ...
        if (interval_ms && i + 1u < samples && h->delay_us) h->delay_us((uint32_t)interval_ms * 1000u);
    }
    *outlen = need;
    return 0;
}

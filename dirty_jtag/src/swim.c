...
int dj_swim_phy_register(const struct dj_swim_phy_ops *ops) {
	if (!ops || !ops->send_packet || !ops->recv_packet)
		return -EINVAL;
	packet_phy = ops;
	return 0;
}
...
const char *dj_swim_phy_name(void) {
	if (use_packet_phy && packet_phy && packet_phy->name)
		return packet_phy->name;
...
int dj_swim_mock_read(uint32_t addr, uint8_t *data, size_t len) {
	if (!data || addr + len > sizeof(mock_mem))
		return -EINVAL;
...
int dj_swim_read_mem(uint32_t addr, uint8_t *data, size_t len) {
	...
	if (!attached)
		return -ENODEV;
	if (mock_enabled)
		return dj_swim_mock_read(addr, data, len);
	...
	while (off < len) {
		...
		r = write_byte_retry((uint8_t)(a >> 16));
		if (r)
			return r;
		r = write_byte_retry((uint8_t)(a >> 8));
		if (r)
			return r;
		r = write_byte_retry((uint8_t)a);
		if (r)
			return r;
...
int dj_swim_write_mem(uint32_t addr, const uint8_t *data, size_t len) {
	...
	if (!attached)
		return -ENODEV;
	if (mock_enabled)
		return dj_swim_mock_write(addr, data, len);
...
int dj_swim_set_speed(bool high_speed) {
	...
	if (high_speed)
		csr |= STM8_SWIM_CSR_HS;
	else
		csr &= (uint8_t)~STM8_SWIM_CSR_HS;
...
int dj_stm8_debug_status(uint8_t *csr1, uint8_t *csr2, bool *halted) {
	...
	if (csr2)
		*csr2 = c2;
	if (halted)
		*halted = (c2 & STM8_DM_CSR2_STALL) != 0u;
	return 0;
}
...
int dj_stm8_debug_set_breakpoint(uint32_t address, uint8_t type, uint8_t slot) {
	if (slot > 1u || address > 0xFFFFFFu)
		return -EINVAL;
	if (type != 0u)
		return -ENOTSUP; /* Phase-D implements instruction-fetch BP. */
	...
	if (r)
		return r;
	if (slot == 0u)
		cr1 = (uint8_t)((cr1 & 0xc7u) | STM8_DM_CR1_BC_IFETCH_OR);
	else
		cr1 = (uint8_t)((cr1 & 0xc7u) | STM8_DM_CR1_BC_IFETCH_OR);
...
int dj_stm8_debug_clear_breakpoint(uint8_t slot) {
	...
	uint8_t b1[3], b2[3];
	r = dj_swim_read_mem(STM8_DM_BK1E, b1, sizeof(b1));
	if (r)
		return r;
	r = dj_swim_read_mem(STM8_DM_BK2E, b2, sizeof(b2));
	if (r)
		return r;

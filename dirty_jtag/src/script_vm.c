...
#define VM_OP_CLOCK_BITS 0x07u /* clk,out,in,bits:u8 flags:u8 hz:u32 tx:ceil(bits/8) -> rx if flags bit1 */
...
{
    if (!out || !pos || !data) return -EINVAL;
    if (*pos + len > cap) return -ENOSPC;
...
{
    switch (d) {
    case 0: return DJ_DIR_INPUT;
    case 1: return DJ_DIR_OUTPUT;
    case 2: return DJ_DIR_OD_LOW;
    default: return DJ_DIR_RELEASE;
    }
}

int dj_script_execute(const uint8_t *s, size_t n, uint8_t *out, size_t *out_len)
{
    if (!s || !out_len) return -EINVAL;
    ...
    while (ip < n) {
        ...
        case VM_OP_END:
            *out_len = op;
            return 0;
        case VM_OP_DIR:
            if (ip + 2u > n || !h->dir) return -EINVAL;
            r = h->dir((enum dj_pin_role)s[ip], vm_dir(s[ip + 1]));
            ip += 2u;
            break;
        case VM_OP_WRITE:
            if (ip + 2u > n || !h->write) return -EINVAL;
            r = h->write((enum dj_pin_role)s[ip], s[ip + 1] != 0u);
            ip += 2u;
            break;
        case VM_OP_READ: {
            if (ip + 1u > n || !h->read) return -EINVAL;
            bool v = false;
        ...
        case VM_OP_DELAY_US:
            if (ip + 4u > n || !h->delay_us) return -EINVAL;
            h->delay_us(le32(s + ip));
            ip += 4u;
            break;
        case VM_OP_POWER:
            if (ip + 1u > n || !h->power) return -EINVAL;
            r = h->power((enum dj_power_mode)s[ip++]);
            break;
        case VM_OP_VPP:
            if (ip + 2u > n) return -EINVAL;
            if (s[ip] == 0u) r = h->vpp_boost ? h->vpp_boost(s[ip + 1] != 0u) : -ENOTSUP;
            else if (s[ip] == 1u) r = h->vpp_apply ? h->vpp_apply(s[ip + 1] != 0u) : -ENOTSUP;
            else if (s[ip] == 2u) r = h->hv_data0_apply ? h->hv_data0_apply(s[ip + 1] != 0u) : -ENOTSUP;
            else r = -EINVAL;
            ip += 2u;
            break;
        case VM_OP_CLOCK_BITS: {
            if (ip + 9u > n || !h->clock_bits) return -EINVAL;
            ...
            uint8_t bits = s[ip++];
            uint8_t flags = s[ip++];
            uint32_t hz = le32(s + ip); ip += 4u;
            if (bits == 0u || bits > 32u) return -EINVAL;
            size_t bytes = (bits + 7u) / 8u;
            if (ip + bytes > n) return -EINVAL;
            uint8_t rx[4] = {0};
            bool want_rx = (flags & 2u) != 0u;
            r = h->clock_bits(clk, dout, din, s + ip, want_rx ? rx : NULL, bits,
                              (flags & 1u) != 0u, hz ? hz : 100000u);
            ip += bytes;
            if (!r && want_rx) r = append(out, &op, cap, rx, bytes);
            break;
        }
        case VM_OP_SPI: {
            if (ip + 6u > n) return -EINVAL;
            uint32_t hz = le32(s + ip); ip += 4u;
            uint16_t len = le16(s + ip); ip += 2u;
            if (ip + len > n || op + len > cap) return -EINVAL;
        ...
        case VM_OP_UART1W: {
            if (ip + 9u > n) return -EINVAL;
            uint32_t baud = le32(s + ip); ip += 4u;
            uint16_t txc = le16(s + ip); ip += 2u;
            uint16_t rxc = le16(s + ip); ip += 2u;
            uint8_t flags = s[ip++];
            if (ip + txc > n || op + rxc > cap) return -EINVAL;
            enum dj_uart_parity parity = (flags & 1u) ? DJ_PARITY_EVEN : DJ_PARITY_NONE;
            uint8_t stop_bits = (flags & 2u) ? 2u : 1u;
            r = dj_uart_1wire_txrx_ex(s + ip, txc, out + op, rxc,
                                      baud ? baud : 115200u, false, parity, stop_bits);
            ip += txc;
            if (!r) op += rxc;
            break;
        }

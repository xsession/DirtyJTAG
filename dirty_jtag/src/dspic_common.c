...
void dj_icsp_bind(const struct dj_icsp_hal *hal) { g_hal = hal; }
const struct dj_icsp_hal *dj_icsp_hal(void) { return g_hal; }

int dj_icsp_six(uint32_t instruction, uint32_t hz)
{
    if (!g_hal || !g_hal->write_bits) return -ENODEV;
...
int dspic_exec_hz(uint32_t instruction, uint32_t hz) { return dj_icsp_six(instruction, hz); }
int dspic_exec(uint32_t instruction) { return dj_icsp_six(instruction, 1000000u); }
...
int dspic_exit_reset(uint32_t goto_insn, unsigned nops_before, unsigned nops_after)
{
    ...
    if (nops_before == 0 && nops_after == 0) {
        ...
        if ((rc = dspic_exec(goto_insn))) return rc;
        if ((rc = dspic_exec(goto_insn))) return rc;
        return dspic_exec(DSPIC_NOP);
    }
    for (unsigned i = 0; i < nops_before; ++i) if ((rc = dspic_exec(DSPIC_NOP))) return rc;
    if ((rc = dspic_exec(goto_insn))) return rc;
    for (unsigned i = 0; i < nops_after; ++i) if ((rc = dspic_exec(DSPIC_NOP))) return rc;
    return 0;
}
...
int dspic_table_load4(unsigned nop_count)
{
    ...
    if (rc) return rc;
    if ((rc = dspic_exec(DSPIC_NOP))) return rc;
    for (size_t i = 0; i < DJ_ARRAY_SIZE(tbl); ++i) {
        if ((rc = dspic_exec(tbl[i]))) return rc;
                       ...
                       unsigned tbl_nops)
{
    ...
    if (rc) return rc;
    if ((rc = dspic_exec(0xEB0380u))) return rc; /* CLR W7 */
    if (goto_insn == DSPIC_GOTO_200 && (rc = dspic_exec(DSPIC_NOP))) return rc;
    for (size_t i = 0; i < DJ_ARRAY_SIZE(rd); ++i) {
        if ((rc = dspic_exec(rd[i]))) return rc;
        for (unsigned n = 0; n < tbl_nops; ++n)
            if ((rc = dspic_exec(DSPIC_NOP))) return rc;
    }
    uint16_t w[6];
    for (unsigned i = 0; i < 6; ++i) {
        if ((rc = dspic_exec(mov_visi_base + i))) return rc;
        if ((rc = dspic_exec(DSPIC_NOP))) return rc;
        if ((rc = dj_icsp_regout(&w[i], 1000000u))) return rc;
        if ((rc = dspic_exec(DSPIC_NOP))) return rc;
    }
    dspic_unpack4(w, out);
    return 0;
                  ...
                  unsigned timeout_ms, unsigned extra_nops)
{
    ...
    if (!hal) return -ENODEV;
    for (unsigned ms = 0; ms < timeout_ms; ++ms) {
        int rc = dspic_exec(DSPIC_NOP); if (rc) return rc;
        rc = dspic_exec(mov_nvmcon_w0); if (rc) return rc;
        rc = dspic_exec(DSPIC_NOP); if (rc) return rc;
        rc = dspic_exec(mov_w0_visi); if (rc) return rc;
        rc = dspic_exec(DSPIC_NOP); if (rc) return rc;
        uint16_t nvmcon = 0;
        rc = dj_icsp_regout(&nvmcon, 1000000u); if (rc) return rc;
        for (unsigned n = 0; n < extra_nops; ++n) if ((rc = dspic_exec(DSPIC_NOP))) return rc;
        rc = dspic_exit_reset(goto_insn, goto_insn == DSPIC_GOTO_200 ? 3 : 0,
                              goto_insn == DSPIC_GOTO_200 ? 3 : 0);
        if (rc) return rc;
        if ((nvmcon & 0x8000u) == 0u) return 0;
        hal->delay_us(1000);
    }
    return -ETIMEDOUT;
}

...
static int read_header(uint32_t cb_addr, uint32_t *max_up, uint32_t *max_down) {
	...
	if (r)
		return r;
	if (memcmp(id, DJ_RTT_ID, DJ_RTT_ID_LEN) != 0)
		return -ENOENT;
	...
	if (r)
		return r;
	if (*max_up > DJ_RTT_MAX_CHANNELS || *max_down > DJ_RTT_MAX_CHANNELS)
		return -ERANGE;
	return 0;
}
...
static uint32_t down_desc_addr(uint32_t cb_addr, uint32_t max_up, uint32_t channel) {
	return cb_addr + DJ_RTT_CB_HEADER_SIZE + max_up * DJ_RTT_BUFFER_DESC_SIZE +
	       channel * DJ_RTT_BUFFER_DESC_SIZE;
}
...
static size_t desc_used(const struct rtt_desc *d) {
	if (!d || d->size == 0u || d->wr >= d->size || d->rd >= d->size)
		return 0u;
	if (d->wr >= d->rd)
		return (size_t)(d->wr - d->rd);
	return (size_t)(d->size - d->rd + d->wr);
}

static size_t desc_free(const struct rtt_desc *d) {
	if (!d || d->size < 2u || d->wr >= d->size || d->rd >= d->size)
		return 0u;
	if (d->rd > d->wr)
		return (size_t)(d->rd - d->wr - 1u);
...
static void safe_name(uint32_t addr, char *out, size_t cap, uint8_t *name_len) {
	uint8_t tmp[32];
	size_t n = 0u;
	if (!out || cap == 0u || !name_len)
		return;
	out[0] = 0;
	*name_len = 0u;
	if (!addr)
		return;
	if (read_target(addr, tmp, sizeof(tmp)) != 0)
		return;
...
int dj_rtt_get_channel_info(uint32_t cb_addr, uint8_t direction, uint8_t channel,
                            struct dj_rtt_channel_info *info) {
	uint32_t max_up = 0u, max_down = 0u;
	uint32_t daddr;
	struct rtt_desc d;
	int r;
	...
	if (r)
		return r;
	if (direction == 0u) {
		if ((uint32_t)channel >= max_up)
			return -ERANGE;
		daddr = up_desc_addr(cb_addr, channel);
	} else if (direction == 1u) {
		if ((uint32_t)channel >= max_down)
			return -ERANGE;
...
int dj_rtt_read_up(uint32_t cb_addr, uint8_t channel, uint8_t *out, size_t *len) {
	...
	*len = 0u;
	...
	if (r)
		return r;
	if (d.size == 0u || d.wr >= d.size || d.rd >= d.size)
		return -ERANGE;
	if (d.wr >= d.rd)
		available = (size_t)(d.wr - d.rd);
	else
		available = (size_t)(d.size - d.rd + d.wr);
	if (available > cap)
		available = cap;
	if (available == 0u)
		return 0;
...
int dj_rtt_write_down(uint32_t cb_addr, uint8_t channel, const uint8_t *data, size_t len,
                      size_t *written) {
	uint32_t max_up = 0u, max_down = 0u;
	struct rtt_desc d;
	uint32_t daddr;
	...
	*written = 0u;
	...
	if (r)
		return r;
	if (d.size < 2u || d.wr >= d.size || d.rd >= d.size)
		return -ERANGE;
	if (d.rd > d.wr)
		free_space = (size_t)(d.rd - d.wr - 1u);
	else
		free_space = (size_t)(d.size - d.wr + d.rd - 1u);
	if (free_space > len)
		free_space = len;
	if (free_space == 0u)
		return 0;

...
static int enter33f(const struct dj_device *dev) {
	...
	if ((r = dj_mclr_low(true)))
		return r;
	h->delay_us(1000);
	if ((r = h->write_bits(0x4D434851u, 32, true, ICSP_HZ)))
		return r;
	h->delay_us(10);
	if ((r = dj_mclr_low(false)))
		return r;
...
static int erase33f(const struct dj_device *d) {
	...
	if (r)
		return r;
	if ((r = dspic_exec(0x2404FAu)))
		return r;
	if ((r = dspic_exec(0x883B0Au)))
		return r;
	if ((r = dspic_exec(0xA8E761u)))
		return r;
	for (int i = 0; i < 4; i++) {
		if ((r = dspic_exec(0)))
			return r;
...
static int write33f_words(const struct dj_device *d, uint32_t a, const uint32_t *w, size_t n) {
	if (!w || n != d->row_words || (a & 1u) || (a % (2u * d->row_words)))
		return -EINVAL;
	if (a + 2u * (uint32_t)n - 2u > d->user_end_pc)
		return -ERANGE;
	int r = dspic_exit_reset(DSPIC_GOTO_200, 0, 0);
	if (r)
		return r;
	if ((r = dspic_exec(0x24001Au)))
		return r;
	if ((r = dspic_exec(0x883B0Au)))
		return r;
	if ((r = dspic_set_tbl_address(a, 7, 0x880190u)))
		return r;
	for (size_t i = 0; i < n; i += 4) {
		uint16_t p[6];
		dspic_pack4(&w[i], p);
		if ((r = dspic_load_wregs_0_5(p)))
			return r;

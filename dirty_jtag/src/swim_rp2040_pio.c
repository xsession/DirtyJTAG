...
{
    if (!ctx.bound) return -ENODEV;
    if (requested_hz < 100000u || requested_hz > 8000000u) return -ERANGE;
    ctx.hz = requested_hz;
...
static int pio_recv_packet(uint8_t *value)
{
    ...
    r = pio_recv_bit(&bit);
    if (r) return r;
    if ((bit ? 1u : 0u) != parity) return -EBADMSG;
...
int dj_swim_rp2040_pio_try_bind(uint8_t data0_gpio, uint32_t initial_hz)
{
    ...
    if (r) return r;
    if (!pio_can_add_program(pio, &swim_tx_program)) return -EBUSY;
    if (!pio_can_add_program(pio, &swim_rx_program)) return -EBUSY;
